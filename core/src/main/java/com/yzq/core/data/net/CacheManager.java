...
import com.yzq.core.data.callback.CacheCallback;
...
public final class CacheManager {
    ...
    private CacheManager() {
        File diskCacheDir = getDiskCacheDir(CoreApp.getAppContext(), CACHE_DIR);
        ...
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        getAppVersion(CoreApp.getAppContext()), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
